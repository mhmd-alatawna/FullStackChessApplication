...
The automated test suite covers signup/session/logout, access control, profile/user operations,
waiting/cancellation, two-human match room creation, legal moves and broadcasts, reconnect/join/sync,
human-to-agent-to-human updates, persisted final agent state, resignation, timeout, statistics/Elo,
agent reconstruction, chess special moves, heuristic/Monte Carlo legal move selection, strong
search timing, and external-engine fallback behavior.
