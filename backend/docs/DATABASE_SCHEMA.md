...
Backend v2 uses Sequelize 6 with MySQL. Application code opens persistence with one public adapter:

```js
const config = require("./src/config");
const SequelizeDataAccess = require("./src/infrastructure/sequelize/SequelizeDataAccess");
const dataAccess = new SequelizeDataAccess(config);
...
`SequelizeDataAccess.initialize()` connects to MySQL, creates `database_name` when missing, and
applies every pending migration recorded by `SequelizeMeta`. It then inserts initial records when
`players` is empty and fills missing built-in account defaults without
...
- `src/infrastructure/sequelize/SequelizeDataAccess.js` is the application-facing adapter. It exposes
  repositories, transaction-scoped repositories, initialization, and shutdown.
- `src/infrastructure/sequelize/connection.js` is the single source for Sequelize connection options
  and database creation used by both runtime startup and migration commands.
- `models/` contains one Sequelize definition per database table. `models/index.js` composes their
  associations without mixing persistence schemas with the domain models under `src/models/`.
- `migrations/schema/` contains the ordered physical schema history. The first migration creates all
  nine application tables, keys, constraints, and indexes. `migrations/MigrationRunner.js` applies
  those files during application startup while recording them in `SequelizeMeta`.
- `seeders/ensureInitialData.js` owns idempotent development accounts and automated-player records.
- Repositories continue to contain data-access queries only.

Normal startup applies only pending migrations. No separate schema command is required for a fresh
