...
function createApplication() {
  ...
  const dataAccess = new Database(applicationConfig);
  const logger = createLogger(false);
  const corsOrigin = applicationConfig.cors_origin;
  ...
  const lifecycle = new ApplicationLifecycle(timeoutScheduler, webSocketGateway);
  ...
  const ready = Promise.resolve(dataAccess.initialize()).then(() => lifecycle.start());
