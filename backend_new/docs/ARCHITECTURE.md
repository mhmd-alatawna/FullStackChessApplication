...
- `src/createApplication.js` is the single composition root. It creates the database, repositories,
  use cases, Express, Socket.IO, routes, scheduler, gateway, and lifecycle.
- `src/server.js` only starts the composed application and handles `SIGINT`/`SIGTERM`.
- `src/bootstrap/ApplicationLifecycle.js` coordinates start restoration and stop cleanup.

These files are complementary: composition, process execution, and lifecycle ownership are separate
...
The executable server creates one `Database` with MySQL. A small JavaScript contract check defines
the required users, games, sessions, matchmaking, agents, IDs, and transaction methods.
...
reject stale writes with `GAME_CONFLICT`. End-to-end tests use the same MySQL adapter as the running
server. See [DATABASE_SCHEMA.md](./DATABASE_SCHEMA.md).
...
1. the MySQL adapter creates the configured database if missing, synchronizes its tables, and adds
   initial data when empty;
...
Synchronization does not alter an existing table definition or delete data.
...
Production work is listed in [DEFERRED_WORK.md](../DEFERRED_WORK.md).
